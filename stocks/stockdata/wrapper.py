...
class StockData():
    ...
    def __init__(self, code):
        self.VALID_INTERVALS = ["1m", "2m", "5m", "15m", "30m", "90m",
    ...
    def _valid_interval(self, period, interval):
        try:
            interval_idx = self.VALID_INTERVALS.index(interval)
            period_idx = self.VALID_PERIODS.index(period)
    ...
    def get_last_price(self):
        hist = self.get_history(period=self.VALID_PERIODS[0], interval=self.VALID_INTERVALS[0])
